...
using Blab.Identity.Exceptions;

namespace Blab.Identity.Cors;
...
internal static class ServiceCollectionExtensions
{
    ...
    internal static IServiceCollection AddCorsServices(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        if (endpointConfig?.Api is null)
        {
            throw new MissingConfigurationException(
                "The 'Api' property could be read from the 'EndpointConfig' configuration section.");
        }
        ...
        return services.AddCors(options =>
        {
            ...
            {
                builder.WithOrigins(endpointConfig.Ui.OriginalString, endpointConfig.Api.OriginalString);
