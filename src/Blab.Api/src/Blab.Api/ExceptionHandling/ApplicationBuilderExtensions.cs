...
{
    internal static IApplicationBuilder UseExceptionHandling(this IApplicationBuilder app, ILoggerFactory loggerFactory) =>
        app.ConfigureExceptions(loggerFactory, builder =>
        {
            ...
            builder
                .DomainException()
                .WithDefaultLogging((ILogger logger, Exception exception) => logger.LogError(exception, exception.Message));
        });
}
